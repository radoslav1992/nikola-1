...
export function renderHome({ lang, data, env, testimonials = null }) {
  ...
  const categories = CATEGORIES.filter((c) => !c.hidden).map((c) => {
...
<section id="search" class="wrap search-section">
  <div class="panel search-panel">
    ...
    <div class="ai-search">
      ...
      <div class="chips chips-sm search-examples">${t.aiExamples.map((q) => html`<button type="button" class="pill" data-search-example>${q}</button>`)}</div>
...
<section id="regions" class="wrap section">
  <div class="section-head"><div><h2>${t.regTitle}</h2><p class="lead-sm">${t.regSub}</p></div></div>
...
<section id="contact" class="wrap section contact-section">
  <div class="two-col">
    ...
    <div><div class="panel">${contactForm(lang)}</div><p class="seller-entry">${t.sellerTitle} <a href="${href(lang, '/predlozhete-imot')}">${t.sellerLink} →</a></p></div>
  </div>
</section>`;
